...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
  ...
  public void filter(ContainerRequestContext request) throws IOException {
    ...
    if (isSecuredResource()) {
      ...
      //If request was authenticated, 
      //generate a Principal and a SecurityContext instance to be user later in the request chain (when our rest endpoint code is called) for authorization
      try {
        UserPrincipal user = getUserPrincipalFromTokenIfValid(token);
        //What if the client had logged out???? ((no token))?
  ...
  //Checks for DenyAll.class, PermitAll.class, RolesAllowed.class
